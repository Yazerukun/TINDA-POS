...
## v1.0.3 IN-APP UPDATE SYSTEM (Phase 23 — 2026-09-06) — RELEASED

> Safe in-app update system (check / download / install, What's New, safety backup,
> portable-update flow) built on the v1.0.2 Hotfix 1 base. Source version bumped to
> `1.0.3`. Gates green: `lint` 0/0 · `typecheck` OK · `test` 119/119 (was 71) ·
> production build PASS · `docs:pdf` 11 pages / final regenerated artifact / version 1.0.3.

### What's implemented
- `electron-updater` (6.8.9, runtime dep) + GitHub `publish` config → `latest.yml`/blockmap emitted
- `src/shared/update.ts` — semver compare, GitHub release parse, stable-only filter, official-URL guard (github.com / api.github.com only), 24h auto-check throttle
- `src/main/services/updateService.ts` — state machine IDLE→CHECKING→UP_TO_DATE/UPDATE_AVAILABLE/DOWNLOADING/DOWNLOADED/READY_TO_INSTALL/DISMISSED/OFFLINE/UNABLE_TO_CHECK/ERROR
- `updateTransport.ts` — net.fetch (15s timeout), streaming portable download → `Downloads\TINDA-POS-Updates\`, pre-update safety backup (`BEFORE_UPDATE`), `restartAndInstall`
- `updateStore.ts` / `updateRuntime.ts` — persisted `{lastCheckedAt,dismissedVersion}` in `updates/update-state.json` (separate from business DB); singleton broadcast to all windows
- `operationGuard.ts` — critical ops (checkout, refund, void, backup, reset…) block an update install mid-operation
- IPC `update:*` + preload API + zustand store + `UpdateNotification` card + **Software Update** section in Settings → About
- Portable never self-overwrites: downloads `TindaPOS-Portable-*.exe` to Downloads then **Show in folder**; portable EXE still uses `%APPDATA%\TINDA POS` unless the user switches to **Portable Data Mode** (existing documented behavior)
...
### WINE QA (Omarchy)
- Packaged app launches under Wine 11.16 (portable wrapper + win-unpacked), no crash
- DB + migrations autogenerated: 28 tables, WAL/VACUUM active; better-sqlite3 win32 native module loads; auto backup file created on ready
...
- **Limit**: `PORTABLE_EXECUTABLE_DIR` is not propagated into Electron's env by the Wine process launcher, so the portable `TindaPOS-Data\` staging + *Show in folder* path was verified by unit tests only; the first complete production updater replacement cycle remains **PENDING** until a newer stable release exists.

### Commit
- `fa29870` feat: in-app update system v1.0.3 — branch `master` (28 files, +2030/−73). Artifacts stay under `source/builds/` (gitignored).
...
> **NATIVE WINDOWS CI QA** (`windows-latest`) green. `v1.0.2-hotfix.1` remains
> preserved as the historical release.
